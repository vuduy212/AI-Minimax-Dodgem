...
Module FindPath
    Public Structure state
        Public current_state As Integer 'trang thai hien tai cua ban co
        Public isBPlaying As Boolean 'quan den co dang choi hay ko
        Public value As Integer 'gia tri cua trang thai hien tai
        Public depth As Integer 'do sau hien tai
        Public Sub New(ByVal current_state As Integer, ByVal isBPlaying As Boolean, ByVal depth As Integer)
            Me.current_state = current_state
            value = -2
            Me.depth = depth
            Me.isBPlaying = isBPlaying
        End Sub
    End Structure
    Public Class NewState
        ...
        Public arr_depth() As Integer 'gia tri tot nhat cua do sau hien tai
        Public Const MaxDepth As Integer = 21 'do sau toi da
        'Cac ham con
        Public Sub New()
            ...
            val_Black(9) = -30
        End Sub
        Public Function is_end_state(ByVal u As state, ByVal depth As Integer) As Boolean
            'Kiem tra xem 1 trang thai co phai la ket thuc hay chua
            If (u.current_state < 100 Or (u.current_state Mod 100 = 0) Or u.depth = depth) Then
                Return True
            End If
            Return False
        End Function
        Public Function find_next_state(ByVal u As state) As Integer
            ...
            next_state.Clear()
            bl1 = u.current_state \ 1000
            bl2 = (u.current_state Mod 1000) \ 100
            re1 = (u.current_state Mod 100) \ 10
            re2 = u.current_state Mod 10
            If (u.isBPlaying) Then 'neu quan den dang choi
                'quan den 1 dang choi
                If (bl1 <> 0) Then
                    If bl1 Mod 3 = 0 Then
                        next_state.Add(New state(u.current_state - 1000 * bl1, False, u.depth + 1))
                    ElseIf bl1 + 1 <> bl2 AndAlso bl1 + 1 <> re1 AndAlso bl1 + 1 <> re2 Then
                        next_state.Add(New state(u.current_state + 1000, False, u.depth + 1))
                    End If

                    If bl1 + 3 <= 9 AndAlso bl1 + 3 <> bl2 AndAlso bl1 + 3 <> re1 AndAlso bl1 + 3 <> re2 Then
                        next_state.Add(New state(u.current_state + 3000, False, u.depth + 1))
                    End If
                ...
                If bl2 <> 0 Then
                    If bl2 Mod 3 = 0 Then
                        next_state.Add(New state(u.current_state - 100 * bl2, False, u.depth + 1))
                    ElseIf bl2 + 1 <> bl1 AndAlso bl2 + 1 <> re1 AndAlso bl2 + 1 <> re2 Then
                        next_state.Add(New state(u.current_state + 100, False, u.depth + 1))
                    End If

                    If bl2 + 3 <= 9 AndAlso bl2 + 3 <> bl1 AndAlso bl2 + 3 <> re1 AndAlso bl2 + 3 <> re2 Then
                        next_state.Add(New state(u.current_state + 300, False, u.depth + 1))
                    End If
            ...
            Else 'quan do dang choi
                ...
                If re1 <> 0 Then
                    If re1 <= 3 Then
                        next_state.Add(New state(u.current_state - 10 * re1, True, u.depth + 1))
                    ElseIf re1 - 3 <> re2 AndAlso re1 - 3 <> bl1 AndAlso re1 - 3 <> bl2 Then
                        next_state.Add(New state(u.current_state - 30, True, u.depth + 1))
                    End If

                    If re1 Mod 3 <> 0 AndAlso re1 + 1 <> re2 AndAlso re1 + 1 <> bl1 AndAlso re1 + 1 <> bl2 Then
                        next_state.Add(New state(u.current_state + 10, True, u.depth + 1))
                    End If
                ...
                If re2 <> 0 Then
                    If re2 <= 3 Then
                        next_state.Add(New state(u.current_state - re2, True, u.depth + 1))
                    ElseIf re2 - 3 <> re1 AndAlso re2 - 3 <> bl1 AndAlso re2 - 3 <> bl2 Then
                        next_state.Add(New state(u.current_state - 3, True, u.depth + 1))
                    End If

                    If re2 Mod 3 <> 0 AndAlso re2 + 1 <> re1 AndAlso re2 + 1 <> bl1 AndAlso re2 + 1 <> bl2 Then
                        next_state.Add(New state(u.current_state + 1, True, u.depth + 1))
                    End If
        ...
        End Function
        Public Sub value_of_state(ByRef u As state)
            Dim i As Integer
            Dim chessman As Integer() = New Integer(3) {}
            'tach vi tri cac quan co
            chessman(0) = u.current_state \ 1000
            chessman(1) = (u.current_state Mod 1000) \ 100
            chessman(2) = (u.current_state Mod 100) \ 10
            chessman(3) = u.current_state Mod 10
        ...
        Private Sub minimax(ByVal u As state, ByRef state_out As Integer)
            If u.isBPlaying Then
                ' tim max
                If arr_depth(u.depth) < u.value Then
                    arr_depth(u.depth) = u.value
                    If u.depth = 1 Then
                        state_out = u.current_state
                    End If
                End If
